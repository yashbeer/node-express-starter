...
const { User, Token, MobileOTP } = require('../../src/models');
...
describe('Auth routes', () => {
  ...
  describe('POST /v1/auth/send-otp', () => {
    ...
    test('should return 429 error if cooldown period is not over', async () => {
      ...
      expect(res.body.message).toMatch(/Please wait \d+ seconds? before requesting another OTP/);
    });
  });
  ...
  describe('POST /v1/auth/verify-otp', () => {
    ...
    test('should return 401 error if mobile number is not found', async () => {
      await request(app)
        .post('/v1/auth/verify-otp')
        .send({
